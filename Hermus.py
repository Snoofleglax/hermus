		...
		if (np.abs(b1[i]-(q[0]*l1[i]**5+q[1]*l1[i]**4+q[2]*l1[i]**3+q[3]*l1[i]**2+q[4]*l1[i]+q[5]))<2):
			if(l1[i]>17):
				if(l1[i]<45):
					...
					newy1.append(y1[i])
					newz1.append(z1[i])
	os.system("touch hermus.1e5.10pc.orb61.2.05B.polyfit.2deg")
	test1=open('hermus.1e5.10pc.orb61.2.05B.polyfit.2deg','r+') #name of output file
	j=0
...
if __name__ == "__main__": 	
	...
	data1=fi.read_file("hermus.1e5.10pc.orb61.2.05B.nbody.params", ",") #name of input nbody file
	
	data2=fi.read_file("hermustest61.forward.250M.params.small", ",") #names of input orbit files
